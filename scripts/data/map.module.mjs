import { objectHp } from "./config.module.mjs";

export const mapObjectBuilders = {
  "country-10": buildCountry10Objects,
  "evil-castle-1": buildEvilCastle1Objects,
  "evil-castle-2": buildEvilCastle2Objects,
};

export function buildMapObjects({ mapDefinition, internalCellCoord, baseObjectHp = objectHp } = {}) {
  const builder = mapObjectBuilders[mapDefinition?.objectLayout];
  if (!builder) return [];
  return builder({ internalCellCoord, objectHp: baseObjectHp });
}

export function buildCountry10Objects({ internalCellCoord, objectHp: baseObjectHp = objectHp }) {
  const objects = [];
...
export function buildEvilCastle1Objects(deps) {
  return buildEvilCastleObjects({
    block033DrawOptions: {
      drawWidthCells: 4.5,
      drawHeightCells: 4,
      drawOffsetX: 10,
...
export function buildEvilCastle2Objects(deps) {
  return buildEvilCastleObjects({
    block033DrawOptions: {
      drawWidthCells: 4.5,
      drawHeightCells: 4,
...
export function buildEvilCastleObjects(layout, { internalCellCoord } = {}) {
  const objects = [];
...
export function summarizeMapObjectBuilders(legacyBuilders = {}) {
