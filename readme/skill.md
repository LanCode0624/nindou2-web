...
- `scripts/main.module.js` now also exposes `globalThis.getNindouModuleProbeReport(options)`; use `{ includeProbe: false }` for a lightweight report, `{ onlyUnsynced: true }` for unsynced sections only, or `{ keysOnly: true }` for probe keys only.
