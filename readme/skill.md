...
- config 的 `ninjutsuRuleProfiles + attackNinjuOutcomeTables + 六顆忍術按鈕 rect + itemSlot/defaultConsumable 常數 + mapItemDrop 常數 + soul/ninjuChain 常數 + 核心戰鬥常數(weapon/maxSkill/objectHp) + NindouConfig` 區段已改成 module 回填：請改 `scripts/data/config.module.mjs` 後跑 `npm run sync:config-nindou` 更新 `scripts/data/config.js` 的 bridge 區段。
