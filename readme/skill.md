...
- 道具背包、掉落、使用與連用流程在 `scripts/systems/consumables.js`；`game.js` 只保留商店 DOM、HUD 繪製與房間協調。
...
- `scripts/systems/consumables.js` 承接道具背包、掉落、使用、連用與 `state.onRoomInventoryChanged` hook；`game.js` 保留商店 DOM 與 HUD 繪製。
