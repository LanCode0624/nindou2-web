...
- 033 障礙物(藍色大火堆，占地六格)：`assets/map/極惡城/1/033-01.png`，key 是 `evilCastleBlock033`。
- 035 障礙物(骨頭)：`assets/map/極惡城/1/035-01.png`，key 是 `evilCastleBlock035`。
- 036 障礙物(紅藍柱)：`assets/map/極惡城/1/036-01.png`，key 是 `evilCastleBlock036`。
...
- 地板：`assets/map/極惡城/2/1.png`，key 是 `evilCastle2Ground`。
- 遮罩：`assets/map/極惡城/2/2.png`，key 是 `evilCastle2Mask`。
...
- `033-01.png` 覆蓋格已針對第二張改成左側 `[3,5]` 到 `[5,4]`、右側 `[14,5]` 到 `[16,4]`。
...
- `033-01.png` 的覆蓋格已改成左側 `[3,5]` 到 `[5,4]`、右側 `[14,5]` 到 `[16,4]`。
...
- `033-01.png` 的視覺 offset 已拆到各自 map layout 內；`極惡城之一` 和 `極惡城之二` 之後要分開調。
...
- 目前地板指向 `assets/map/極惡城/2/1.png`，遮罩指向 `assets/map/極惡城/2/2.png`。
- 預設地圖已切回 `鄉野之十`。
