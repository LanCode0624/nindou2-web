...
- `scripts/main.module.js` 也會輸出 `globalThis.NindouModuleProbeSummary`，含 `total/synced/unsynced/syncedKeys/unsyncedKeys`，可在 browser console 快速判斷是否全部同步。
...
- `scripts/main.module.js` 也會輸出 `globalThis.NindouModuleProbeMeta`，目前含 `version`、`sectionKeys`、`sectionCount`、`syncedCount`、`unsyncedCount`、`hasUnsynced`、`generatedAt`，供後續 probe schema 演進與診斷時間點判斷。
- `scripts/main.module.js` 也會輸出 `globalThis.getNindouModuleProbeReport(options)`；預設回傳 `{ optionsUsed, meta, summary, warnings, probe }`，傳 `{ includeProbe: false }` 可拿精簡報表（不含 `probe` 大物件），傳 `{ includeMeta: false }` 可省略 meta，傳 `{ onlyUnsynced: true }` 可只回傳不同步的 `probe` 區塊，傳 `{ keysOnly: true }` 可只回傳 probe key 陣列。
