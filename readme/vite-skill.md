...
- `scripts/main.module.js` 也會輸出 `globalThis.getNindouModuleProbeReport(options)`；預設回傳 `{ meta, summary, warnings, probe }`，傳 `{ includeProbe: false }` 可拿精簡報表（不含 `probe` 大物件），傳 `{ includeMeta: false }` 可省略 meta，傳 `{ onlyUnsynced: true }` 可只回傳不同步的 `probe` 區塊，傳 `{ keysOnly: true }` 可只回傳 probe key 陣列。
